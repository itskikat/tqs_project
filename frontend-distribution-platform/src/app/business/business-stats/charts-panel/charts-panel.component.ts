...
export class BusinessStatsCharts implements OnDestroy{
  ...
  constructor(private theme: NbThemeService,private providerServiceService: BusinessServiceService, public datepipe: DatePipe ) {
  ...
  getChartPannelSummary(date: Date[]) {
    this.providerServiceService.getBusinessStatistics(this.datepipe.transform(date[0], 'dd/MM/yyyy'), this.datepipe.transform(date[1], 'dd/MM/yyyy'))
      .pipe(takeWhile(() => this.alive))
      .subscribe(data => {
        this.chartPanelSummary=[];
        this.chartPanelSummary.push({title:'Profit', value: data.PROFIT });
        this.chartPanelSummary.push({title:'Number of contracts', value: data.TOTAL_CONTRACTS });
        this.chartPanelSummary.push({title:'Service with most contracts', value: data.MOST_REQUESTED_SERVICETYPE.name });
      });
  }
  ...
  getChartData(date: Date[]) {
    this.providerServiceService.getBusinessStatistics(this.datepipe.transform(date[0], 'dd/MM/yyyy'), this.datepipe.transform(date[1], 'dd/MM/yyyy'))
      ...
      .subscribe(data => {
        ...
        this.ordersChartData.chartLabel=Object.keys(data.PROFIT_HISTORY);
        this.ordersChartData.linesData=Object.values(data.PROFIT_HISTORY).map(Number);
